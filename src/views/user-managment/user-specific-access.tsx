...
import UserSpecificModal from "views/user-managment/modals/user-specific-modal";
...
interface DataRow {
  ...
  name: string;
  scope: string;
  read_write: React.ReactNode;
  read_only: React.ReactNode;
  not_allowed: React.ReactNode;
  subRows?: DataRow[];
  [key: string]: any;
}
...
const sampleData: DataRow[] = [
  { 
    id: 1, 
    name: "johnsmith@demo.com", 
    scope: "OMS", 
    read_write: <CloseCircleOutlined/>, 
    read_only: <CloseCircleOutlined/>, 
    not_allowed: <CheckCircleOutlined/>,
    subRows: [
      { id: 101, name: "Super Admins", scope: "admin", read_write:<CloseCircleOutlined/>, read_only: 2, not_allowed: "John Smith" },
      { id: 102, name: "System Admins", scope: "admin", read_write: <CloseCircleOutlined/>, read_only: 3, not_allowed: "John Smith" },
    ]
  },
  { 
    id: 2, 
    name: "janedoe@demo.com", 
    scope: "OMS", 
    read_write: <CloseCircleOutlined/>, 
    read_only: <CloseCircleOutlined/>, 
    not_allowed: <CheckCircleOutlined/>,
    subRows: [
      { id: 201, name: "Content Editors", scope: "write",read_write: <CloseCircleOutlined/>, 
        read_only: <CloseCircleOutlined/>, 
        not_allowed: <CheckCircleOutlined/>, },
      { id: 202, name: "Review Editors", scope: "write", read_write: <CloseCircleOutlined/>, 
        read_only: <CloseCircleOutlined/>, 
        not_allowed: <CheckCircleOutlined/>,},
    ]
  },
...
// Props interface for the user specific access component
interface UserSpecificAccessProps {
  data?: DataRow[];
  isLoading?: boolean;
  onAdd?: () => void;
...
export default function UserSpecificAccess({
  data = sampleData,
  isLoading = false,
  onAdd,
  onDelete,
  onRefresh,
}: UserSpecificAccessProps) {
  ...
  const handleOpen = (event: React.MouseEvent<HTMLButtonElement>) => {
    setOpenModal(true);
    if (onAdd) onAdd(); // Fixed to properly call the function
  }
  ...
  const columns = useMemo<ColumnDef<DataRow, unknown>[]>(() => [
    ...
    {
      accessorKey: "read_write",
      header: "Read and Write",
      enableSorting: true,
      enableFiltering: true,
      cell: ({ getValue }) => {
              const accessType = getValue() as React.ReactNode;
              let color: 'error' | 'success' | 'default' = 'default';
              
              if (React.isValidElement(accessType)) {
                if (accessType.type === CloseCircleOutlined) {
                  color = 'error';
                } else if (accessType.type === CheckCircleOutlined) {
                  color = 'success';
                }
              }
  ...
  const handleExportCSV = () => {
    ...
    FileSaver.saveAs(blob, "user-specific-access-export.csv");
  };
      ...
      >
        ...
        <Stack direction="row" justifyContent="space-between" width="100%">
          ...
          >
           ...
           <UserSpecificModal open={openModal} handleClose={handleClose}/>
        </Stack>
      </Box>
